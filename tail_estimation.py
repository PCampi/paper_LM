...
def add_uniform_noise(data_sequence, p=1):
    ...
    noise = np.random.uniform(
        -5. * 10**(-p), 5 * 10**(-p), size=len(data_sequence))
    randomized_data_sequence = data_sequence + noise
    # ensure there are no negative entries after noise is added
    randomized_data_sequence = randomized_data_sequence[np.where(
        randomized_data_sequence > 0)]
...
def hill_dbs(ordered_data,
    ...
    while k2 == None:
        ...
        max_index1 = (
            np.abs(np.linspace(1. / n1, 1.0, n1) - eps_stop)).argmin()
        k1 = np.nanargmin(averaged_delta[min_index1:max_index1]
                          ) + 1 + min_index1  #take care of indexing
        if diagn_plots:
        ...
        max_index2 = (
            np.abs(np.linspace(1. / n2, 1.0, n2) - eps_stop)).argmin()
        averaged_delta = samples_n2 / good_counts2

        max_index1 = (
            np.abs(np.linspace(1. / n1, 1.0, n1) - eps_stop)).argmin()
        k2 = np.nanargmin(averaged_delta[min_index2:max_index2]
                          ) + 1 + min_index2  #take care of indexing
        if diagn_plots:
    ...
    return k_star, x1_arr, n1_amse, k1 / float(
        n1), max_index1, x2_arr, n2_amse, k2 / float(n2), max_index2
...
def smooth_hill_estimator(ordered_data, r_smooth=2):
    ...
    bin_lengths = np.array([1.] +
                           [float((r_smooth - 1) * k) for k in k_arr[:-1]])
    cum_sum = 0.0
...
def kernel_type_dbs(ordered_data,
                    hsteps,
                    t_bootstrap=0.5,
                    r_bootstrap=500,
                    alpha=0.6,
    ...
    max_index1 = (np.abs(
        np.logspace(np.log10(1. / n1), np.log10(1.0), hsteps) -
        eps_stop)).argmin()
    ...
    max_index2 = (np.abs(
        np.logspace(np.log10(1. / n2), np.log10(1.0), hsteps) -
        eps_stop)).argmin()
...
def kernel_type_estimator(ordered_data,
                          hsteps,
                          alpha=0.6,
                          bootstrap=True,
                          t_bootstrap=0.5,
    ...
    n = len(ordered_data)
    h_arr, xi_arr = get_biweight_kernel_estimates(
        ordered_data, hsteps, alpha=alpha)
    if bootstrap:
...
def make_plots(ordered_data, output_file_path, number_of_bins, r_smooth, alpha,
               hsteps, bootstrap_flag, t_bootstrap, r_bootstrap, diagn_plots,
               eps_stop, theta1, theta2, verbose, noise_flag, p_noise,
               savedata):
    """ 
    ...
    if savedata == 1:
        with open(
                os.path.join(output_dir + "/" + output_name + "_pdf.dat"),
                "w") as f:
    ...
    if savedata == 1:
        with open(
                os.path.join(output_dir + "/" + output_name + "_ccdf.dat"),
                "w") as f:
    ...
    if savedata == 1:
        with open(
                os.path.join(output_dir + "/" + output_name + "_pickands.dat"),
                "w") as f:
    ...
    t1 = time.time()
    k_sh_arr, xi_sh_arr = smooth_hill_estimator(
        ordered_data, r_smooth=r_smooth)
    t2 = time.time()
    if verbose:
        print("Elapsed time (smooth Hill):", t2 - t1)
    if savedata == 1:
        with open(
                os.path.join(output_dir + "/" + output_name + "_sm_hill.dat"),
                "w") as f:
    ...
    if savedata == 1:
        with open(
                os.path.join(output_dir + "/" + output_name +
                             "_kern_plot.dat"), "w") as f:
            for i in range(len(k_k_arr)):
                f.write(str(k_k_arr[i]) + " " + str(xi_k_arr[i]) + "\n")
        with open(
                os.path.join(output_dir + "/" + output_name +
                             "_kern_estimate.dat"), "w") as f:
            f.write(
                str(k_k_arr[k_k1_star]) + " " + str(xi_k_arr[k_k1_star]) +
                "\n")
    ...
    if (xi_sh_arr[min_k_index] <= -3 or xi_sh_arr[min_k_index] >= 3):
        ...
        indices_to_plot_sh = np.where((k_sh_arr <= max_k) &
                                      (k_sh_arr >= min_k))
    ...
    if diagn_plots:
        ...
        axes_d[0].set_xlabel(
            "Fraction of Bootstrap Order Statistics", fontsize=20)
        ...
        if savedata == 1:
            ...
            with open(
                    os.path.join(output_dir + "/" + output_name +
                                 "_adjhill_diagn_points.dat"), "w") as f:
                f.write("Min for n1 sample: " + str(k1_h) + " " +
                        str(n1_h_amse[int(len(x1_h_arr) * k1_h) - 1]) + "\n")
                f.write("Min for n2 sample: " + str(k2_h) + " " +
                        str(n2_h_amse[int(len(x2_h_arr) * k2_h) - 1]) + "\n")
                f.write("Minimization boundary for n1 sample: " +
                        str(max_h_index1 / float(len(x1_h_arr))) + "\n")
                f.write("Minimization boundary for n2 sample: " +
        ...
        if savedata == 1:
            ...
            with open(
                    os.path.join(output_dir + "/" + output_name +
                                 "_mom_diagn_points.dat"), "w") as f:
                f.write("Min for n1 sample: " + str(k1_m) + " " +
                        str(n1_m_amse[int(len(x1_m_arr) * k1_m) - 1]) + "\n")
                f.write("Min for n2 sample: " + str(k2_m) + " " +
        ...
        axes_d[2].plot(
            ...
            color="#d55e00",
            label=r"$n_1$ samples")
        axes_d[2].scatter([h1], [n1_k_amse[np.where(x1_k_arr == h1)]],
                          color="#d55e00",
                          marker='o',
                          edgecolor="black",
                          alpha=0.5,
        ...
        axes_d[2].plot(
            ...
            color="#0072b2",
            label=r"$n_2$ samples")
        axes_d[2].scatter([h2], [n2_k_amse[np.where(x2_k_arr == h2)]],
                          color="#0072b2",
                          marker='o',
                          edgecolor="black",
                          alpha=0.5,
        ...
        if savedata == 1:
            ...
            with open(
                    os.path.join(output_dir + "/" + output_name +
                                 "_kern_diagn_points.dat"), "w") as f:
                f.write("Min for n1 sample: " + str(h1) + " " +
                        str(n1_k_amse[np.where(x1_k_arr == h1)][0]) + "\n")
                f.write("Min for n2 sample: " + str(h2) + " " +
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Script to compute tail index estimates\
                                     for a provided dataset.")
    parser.add_argument(
        "sequence_file_path", help="Path to a data sequence.", type=str)
    parser.add_argument(
        "output_file_path",
    ...
    if args.theta1 < 0.0 or args.theta1 > 1.0:
        parser.error(
            "Theta parameters should be in [0,1] range, where theta1 < theta2."
        )
    ...
    with open(args.sequence_file_path, "r") as f:
        for line in f:
            degree, count = line.strip().split(delimiter)
            ordered_data[current_index:current_index +
                         int(count)] = float(degree)
    ...
    print(
        "Selected fraction of order statistics boundary for AMSE minimization: %0.4f"
        % eps_stop)
    print("========================")
    make_plots(ordered_data, args.output_file_path, number_of_bins, r_smooth,
               alpha, hsteps, bootstrap_flag, t_bootstrap, r_bootstrap,
               diagnostic_plots_flag, eps_stop, args.theta1, args.theta2,
               verbose, noise_flag, p_noise, args.savedata)
